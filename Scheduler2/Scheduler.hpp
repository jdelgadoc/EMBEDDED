...
public:
    ...
    void    ProcessSMS(SMS iNewSMS);
    void    GetMessage(void);
    void    AttachMessage(SMS iNewSMS);
    void    ClearNextSchedule(void);
    void    ClearSMSQ(void);
    uint8_t run(void);
    uint8_t setup(void);
private:
    uint8_t m_u8OpenSlots; // - Available slots
    uint8_t m_u8SMSIndex;
    ...
    SMS m_aSMSQ[NUMBER_OF_SLOTS];
     // - Calculate next schedule tasks (not implemented)
